...
const emissionsReport = async (data) => {
  try {
    ...
    let queryInitial = `
      ...
      (select top(1) ptasamon_cobro from cbrecibos where cvigencia = b.cvigencia order by fcobro desc) ptasamon_cobro,
      ...
      (select top(1) iestadorec from cbrecibos where cvigencia = b.cvigencia order by fcobro desc) iestadorec,
    ...
    const formatResult = await result.recordset.map(item => {
      return {
        ...
        "Estatus Última Cuota": item.iestadorec || 'P',
        "Tasa de Cobro": item.ptasamon_cobro || null,
        "Nº de Cuotas Pagadas": item.ncuotas_pagadas || 0,
        "Última Fecha de Cobro": item.ult_fcobro?.toLocaleDateString('en-GB') || null,
